...
// FuzzCoarsenedLeiden mirrors FuzzLeiden for the multi-level entry
...
func FuzzCoarsenedLeiden(f *testing.F) {
	f.Add(int8(8), []byte{0, 0, 1, 0, 16, 0, 1, 0, 2, 0, 16, 0}, 0.1, int64(1))
	...
	f.Fuzz(func(t *testing.T, nNodes int8, edgeBlob []byte, gamma float64, seed int64) {
		...
		res, err := CoarsenedLeiden(context.Background(), n, decodeEdges(edgeBlob, n), opts)
		if err != nil {
			return
		}
		if len(res.Levels) == 0 {
			t.Fatalf("Levels is empty after successful CoarsenedLeiden")
		}
