...
// TestValidation_CoarsenedLeiden_LevelsMatchSingleLeiden checks that
...
// Final quality of CoarsenedLeiden should track Leiden's Final
...
func TestValidation_CoarsenedLeiden_LevelsMatchSingleLeiden(t *testing.T) {
	n, edges := mustLoadEdges(t, "testdata/karate.edges")
	...
	for _, seed := range []int64{1, 7, 17, 42, 99} {
		...
		hier, err := CoarsenedLeiden(context.Background(), n, edges, opts)
		if err != nil {
			t.Fatalf("CoarsenedLeiden: %v", err)
		}
	...
	if rel > 0.02 {
		t.Errorf("best Leiden Q and best CoarsenedLeiden Q differ by %.6g (relative %.4f), want ≤2%%",
			dq, rel)
	}
}
...
func TestValidation_QualityMonotonicAcrossLevels(t *testing.T) {
	...
	opts.Resolution = 0.1
	opts.Seed = 5
	res, err := CoarsenedLeiden(context.Background(), n, edges, opts)
	if err != nil {
		t.Fatalf("CoarsenedLeiden: %v", err)
	}
