...
var (
	...
	// ErrInvalidObjective is returned when Options contains an unknown objective.
	ErrInvalidObjective = errors.New("leiden: invalid objective")

	// ErrNodeWeightsWithModularity is returned because modularity derives node
	// masses from weighted degree and cannot accept caller-provided masses.
	ErrNodeWeightsWithModularity = errors.New("leiden: node weights are only supported by the CPM objective")

	// ErrInvalidMaxClusterSize is returned when hierarchical clustering is
	// configured with a non-positive size threshold.
	ErrInvalidMaxClusterSize = errors.New("leiden: max cluster size must be positive")
)
