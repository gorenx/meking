...
func runRefinement(net *CompactNetwork, parent *Clustering, q qualityFunction, theta float64, rng *rand.Rand) *Clustering {
	cl, _ := runRefinementContext(context.Background(), net, parent, q, theta, rng)
	return cl
}

func runRefinementContext(ctx context.Context, net *CompactNetwork, parent *Clustering, q qualityFunction, theta float64, rng *rand.Rand) (*Clustering, error) {
	n := net.nNodes
	gamma := q.resolution()
	...
	// Scratch state, reused across iterations.
