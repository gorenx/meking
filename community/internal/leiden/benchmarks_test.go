...
func BenchmarkLeiden_PlantedPartition_100(b *testing.B) {
	...
	benchmarkLeiden(context.Background(), b, fx)
}
...
// BenchmarkCoarsenedLeiden_PlantedPartition_1k measures the hierarchical
// entry point at medium scale.
func BenchmarkCoarsenedLeiden_PlantedPartition_1k(b *testing.B) {
	fx := makeBenchFixture(b, 250, 0.05, 0.001)
	opts := DefaultOptions()
	...
	for b.Loop() {
		_, err := CoarsenedLeiden(context.Background(), fx.nNodes, fx.edges, opts)
		if err != nil {
			b.Fatalf("CoarsenedLeiden: %v", err)
		}
	}
}
...
func benchmarkLeiden(ctx context.Context, b *testing.B, fx benchPlantedFixture) {
	opts := DefaultOptions()
	opts.Resolution = 0.05
	opts.Seed = 1
	b.ResetTimer()
